...
df <- readRDS(paste(create_dataset_for_regressions_dir, "/data/rdd_data_moderation_", "strict", "definition.rds", sep = ""))
df <- df %>% 
  reframe(id_municipio, coorte, stem_background, sch_non_stem_cdt)
...
sivep_2020 <- read_csv2(paste0(create_covid_data_dir,"/input/220613_srag_base_oficial_2020.csv"), col_types = cols(
...
sivep_2021 <- read_csv2(paste0(create_covid_data_dir,"/input/230831_srag_base_oficial_2021.csv"), col_types = cols(
  DT_NOTIFIC = col_date(format = formato_data),
  SEM_NOT = col_double(),
  DT_SIN_PRI = col_date(format = formato_data),
  SEM_PRI = col_double(),
...
sivep_2020 <- sivep_full %>%
  filter((CLASSI_FIN == "SRAG COVID-19" | CLASSI_FIN == "SRAG não especificado") & DT_SIN_PRI >= "2020-01-01" & DT_SIN_PRI <= "2020-12-30") %>%
...
sivep_2020$grupo = ifelse(sivep_2020$CO_MUN_RES %in% subset(df, stem_background == 1 & coorte == 2016 )$id_municipio, "STEM",
                          ifelse(sivep_2020$CO_MUN_RES %in% subset(df, stem_background == 0 & coorte == 2016 )$id_municipio, "Non-STEM", NaN)) 
